...
function getChefkochData(link, setIsLoading) {
  return new Promise(async (resolve, reject) => {
    try{
      ...
      recipeTitle = mainJQuery.find('article.ds-box.ds-grid-float.ds-col-12.ds-col-m-8.recipe-header > div > h1').text();
      ...
      zutatenHeadline = mainJQuery.find("main > article.ds-box.ds-grid-float.ds-col-12.ds-col-m-8.recipe-ingredients.ds-or-1 > h2 ").text();
      portionSize = mainJQuery.find("main > article.ds-box.ds-grid-float.ds-col-12.ds-col-m-8.recipe-ingredients.ds-or-1 > div.recipe-servings.ds-box > form > input").val();

      //descriptionText
      let descriptionText = $('p.recipe-text')
      description = descriptionText.text().trim().replace(/\s{2,}/g, ' ');

      let categoryText = $('a.ds-tag')
      let categoryArray = categoryText.text().trim().replace(/\s{2,}/g, ' ').split(' ');
      category = categoryArray[0];
      //time
      let timeText = $('span.recipe-preptime')
      time = timeText.text().trim().replace(/\s{2,}/g, ' ').replace('', '').trim();
      ...
      //Zubereitung
      mainJQuery.find("article.ds-box.ds-grid-float.ds-col-12.ds-col-m-8.ds-or-3 > div:nth-child(3)").each((i, el) => {
